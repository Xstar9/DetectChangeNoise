...
public class ChangeFor2While extends ASTVisitor{
	...
	public void endVisit(CompilationUnit node) {
		...
		int random = rand.nextInt(fors.size());
		int i = 0;
		AST ast = cu.getAST();
		...
		for(Expression ini: initexpressions) {//Be careful!!!, we have to rename the local variable
			if(IsNameDeclaration(ini)) {
				...
				for(VariableDeclarationFragment frag: ini_frags) {
					...
					System.out.println("decName: "+decName.getIdentifier());
					...
					//Begin Change
					// Utils.getRandomString(5); // TODO: 这里仅需要把for转为while，不硬性需要对变量重命名
					String newName = decName.getIdentifier();
