...
import static util.CompareSourceCode.cmpTransformLinesBelongMethod;

public class ManyMain {
    public static String projectList = "D:\\JavaProject\\SPAT\\local_project_list.csv";
...
//    public static String commitListPath = projectLocalPath + currentDir + "commitList.csv";
    ...
    public static List<Project> projectCasesList = new ArrayList<>();
...
//                random0 = rand.nextInt(srcLocatedList.size());
                for(int col = 0; col < srcLocatedList.size(); col++){
                    localPath = srcLocatedList.get(col);
                    localPath = localPath.substring(0, localPath.length() - 14);
                    currentDir = rootDir + "/" + localPath.substring(localPath.lastIndexOf("\\") + 1);
...
//            e.printStackTrace();
//        }
        Set<String> transformMethods = new HashSet<>();
        ...
        if (isHasChange) {
            for (File file : fileSet) {
                ...
                if (new File(postPath).exists() && new File(prePath).exists()) {
                    ...
                    // TODO: 比较两个文件的差异，得知变更的行范围，后续关联到属于方法范围内，输出到数据集
                    try {
                        Set<String> changeMethodSet = cmpTransformLinesBelongMethod(prePath,postPath);
                        transformMethods.addAll(changeMethodSet);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
            ...
            try (Git git = Git.open(findGitDirectoryUpwards(new File(dirPath)))) {
                ...
                if (!status.hasUncommittedChanges()) {
                    ...
                    int defaultRelatedTestCaseNum = 0;
                    if(currentProject.getProjectName().equals(currentDir.split("/")[0])){
                        for(String file:changeFiles){
                            Map<String, Integer> expandClassMapCases = currentProject.getExpandClassMapCases(file);
                            for (String s:transformMethods){
                                if(expandClassMapCases.containsKey(s.split(" ")[1])){
                                    defaultRelatedTestCaseNum += expandClassMapCases.get(s);
                                }else {
...
//                        defaultRelatedTestCaseNum = currentProject.getTestCaseSum();
                    ...
                    writeDataToCSV(success_trans_file, new String[]{String.valueOf(count), currentDir,
                            preCommitId, postCommitId, TransformationType.fromCode(Integer.parseInt(idOfRule)),idOfRule,
                                                        "1",    // 转换规则数量
                                                         String.valueOf(transformMethods.size()),transformMethods.toString(),String.valueOf(defaultRelatedTestCaseNum)});
...
//        System.out.println(outputPath);
    ...
    public static void writeDataToCSV(String filePath, String[] data) {
        ...
        try {
            ...
            if (!fileExists) {
                ...
                String[] header = {"id", "projectName", "startCommit", "endCommit", "transformType","transformTypeId", "transformSum", "RelatedMethodsNum","RelatedMethodSet","RelatedCasesNum"};
    ...
    public static void writeCloneErrorToCSV(String filePath, String[] data) {
        File file = new File(filePath);
        boolean fileExists = file.exists();
        CSVWriter writer = null;
