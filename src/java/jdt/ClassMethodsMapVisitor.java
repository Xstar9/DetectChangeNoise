 ...
 * @author Z.X
 */
public class ClassMethodsMapVisitor extends ASTVisitor {
    // think：JDT以一段源码为输入就能访问方法块，因此一般情况下是不易获取到类所在的包（全限定）的
    // 若需要可能需要传入指定的整个程序以及依赖，然后使用JDT的求解器类ResolveBinding（）获取类所在的包

    public static String className;

    public Map<String,List<String>> showClassMethod(String code) {
        Map<String, List<Integer>> methodLocMap = new TreeMap<>();
        Map<String,List<String>> methodLocMap1 = new TreeMap<>();
...
//                            parameters.append(parameterType).append(" ").append(parameterName).append(", ");
                ...
                if(methodLocMap1.containsKey(className)){
                    methodLocMap1.get(className).add(methodSignature);

                }
                else {
                    List<String> list = new ArrayList<>();
                    list.add(methodSignature);
                    methodLocMap1.put(className ,list);
                }
                // 获取方法块的起始行和结束行
